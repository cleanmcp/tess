...
# shared contact helpers live in ../lib (resolve through a symlink if any)
_src="${BASH_SOURCE[0]}"; while [ -L "$_src" ]; do _src="$(readlink "$_src")"; done
TESS_PYLIB="$(cd "$(dirname "$_src")/../lib" && pwd)"

matches="$(N="$name" TESS_PYLIB="$TESS_PYLIB" python3 -c "
import os,sys; sys.path.insert(0,os.environ['TESS_PYLIB'])
from tess_common import find_contacts, find_number
...
if MSG="$msg" NUM="$num" osascript -e 'tell application "Messages"
  ...
  echo "✗ failed — allow Automation (your terminal → Messages) in System Settings → Privacy & Security → Automation."
fi
