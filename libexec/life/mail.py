...
from tess_common import C, resolve, short, contacts_map  # noqa: E402
...
def die_perm():
    print("can't read Mail — grant Full Disk Access to your terminal app, then restart it.")
...
def osa(script, *args):
    ...
    if p.returncode != 0 or err:
        if "-1743" in err or "Not authorized" in err:
            print("Mail actions blocked — allow Automation (your terminal → Mail):")
            print("  System Settings → Privacy & Security → Automation → your terminal → enable Mail.")
...
def resolve_recipient(q):
    ...
    for a, n in contacts_map().items():          # Contacts first…
        if "@" in a and ql in n.lower():
            cands[a.lower()] = (n, a)
    for a, cmt in CON.execute("SELECT DISTINCT address, comment FROM addresses"):  # …then people who've mailed the user
