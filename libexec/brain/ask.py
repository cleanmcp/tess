...
def model_for(tier, default):
    """Local model for a size tier, from ~/.config/tess/models (small=/med=/big=)."""
    try:
        for line in open(os.path.expanduser("~/.config/tess/models")):
...
def ollama(prompt, model=None, system="", timeout=120):
    model = model or model_for("med", "llama3.2:3b")
...
ROUTER = """You are the router for `tess`, the user's personal CLI. Read their request and reply with EXACTLY ONE line, nothing else:

RUN <cmd>        run a tess command. Valid <cmd>: brief | people | companies | reminders | calls | calendar | messages <name> | mail <query> | read
SESSION <name>   they're asking what's going on / status of a past coding session (e.g. "kimi chat fix", "the gcp session")
NOTE <name>      they want a note/person read (e.g. a person or topic)
ANSWER <text>    it's a general question, a word/definition, or about a book they're reading — answer it yourself, briefly and plainly

Rules: pick the single best one. If they say "what's going on" with no session named -> RUN brief. If unsure -> ANSWER."""
...
if verb == "RUN":
    ...
    if texts:
        blob = "\n".join(texts[-25:])[:3000]
        speak(ollama(f"the user asked what's going on in their '{name}' session. From this recent activity, tell them in 2-4 short spoken sentences where it's at and what's next:\n{blob}"))
    else:
