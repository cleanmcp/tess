  ...
  startup)
    case "${1:-run}" in
      run|"")
        ...
        for a in ${TESS_STARTUP_OPEN:-cmux Music Safari}; do open -a "$a" 2>/dev/null && echo "▶ $a"; done ;;
      install)
...
PL
        ...
        echo "✓ installed — your startup set (${TESS_STARTUP_OPEN:-cmux Music Safari}) opens at login."
