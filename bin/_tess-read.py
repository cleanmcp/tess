...
if not dbs:
    die("can't find your Apple Books library (grant Full Disk Access to cmux).")
...
try:
    ...
    die("can't read the library — make sure cmux has Full Disk Access, then restart cmux.")
...
mode += (f"\n\n---\n**Current book:** {title} — {author}. Full text: `{cache_file}`. "
         ...
         f"Apple Books has been opened to his spot. FIRST THING: run `tess readpos` and give him "
         f"the 📍/⏪/⏩ (where you are · recap · coming-up gist), then offer to explain anything.")

# open Apple Books to his current book/position so he reads natively there
