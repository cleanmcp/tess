#!/usr/bin/env python3
# _tess-watch.py — F2 wait + F6 escalation. No stuck sub-agent goes unnoticed.
#
...
def escalate(name, kind, detail, phone):
    ...
    if phone and NOTIFY:
        subprocess.run(["bash", os.path.join(TESS_BIN, "_tess-send.sh"), NOTIFY, "--",
...
def main():
    args = sys.argv[1:]
    if not args:
        die("usage: _tess-watch.py wait|watch …")
    sub = args.pop(0)
    if sub == "wait":
        cmd_wait(args)
    elif sub == "watch":
