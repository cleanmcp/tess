...
#   _tess-repos.sh index                 (re)build the map now
#   _tess-repos.sh list                  ranked repos (most active first)
#   _tess-repos.sh find <name>           matching  name<TAB>path  lines, best first
#   _tess-repos.sh pick <name>           ONE confident match (exit 3 = ambiguous, candidates on stdout)
#   _tess-repos.sh iso <name> [feature]  make/print an isolated worktree for ANY repo
#
...
case "${1:-list}" in
  ...
  iso)
    ...
    out="$(bash "$0" pick "$name")"; rc=$?
    if [ $rc -eq 1 ]; then
      echo "no repo named '$name' found. (tess repos index to rescan, or set TESS_REPO_DIRS)" >&2; exit 1
    elif [ $rc -eq 3 ] && [ -t 0 ] && command -v fzf >/dev/null 2>&1; then
      sel="$(echo "$out" | fzf --prompt 'repo ▸ ' --delimiter '\t')"; [ -z "$sel" ] && exit 0
      repo="${sel#*$'\t'}"
    elif [ $rc -eq 3 ]; then
      echo "ambiguous '$name':" >&2; echo "$out" | sed $'s/\t/  ->  /;s/^/  /' >&2
      echo "use the path instead." >&2; exit 2
    else
      repo="${out#*$'\t'}"
    fi
    base="$(basename "$repo")"
    ...
    if [ ! -d "$dst" ]; then
      ...
      if git -C "$repo" show-ref --verify --quiet "refs/heads/$feature"; then
        git -C "$repo" worktree add "$dst" "$feature" >/dev/null 2>&1
      else
        git -C "$repo" worktree add -b "$feature" "$dst" >/dev/null 2>&1
      fi
    fi
    echo "$dst"
    ;;

  *) echo "usage: $0 [index|list|find <name>|pick <name>|iso <repo> [feature]]" >&2; exit 1 ;;
esac
